...
export function AdCard({ ad, featured = false }: { ad: any, featured?: boolean }) {
  // Use ad.media_urls[0] if it exists, otherwise use a placeholder
      ...
      <Link href={`/ads/${ad.id}`} className="flex-1 flex flex-col relative z-10">
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          <div className="absolute top-4 left-4 flex gap-2">
            {featured && (
              <Badge className="bg-primary hover:bg-primary text-white border-none shadow-lg shadow-primary/40 px-3 py-1 font-bold text-[10px]">
                FEATURED
              </Badge>
            )}
            <Badge variant="secondary" className="bg-black/40 backdrop-blur-md text-white border-white/10 px-3 text-[10px]">
              {ad.category}
            </Badge>
          </div>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col">
          ...
          <div className="flex flex-col gap-3 mb-6 text-[13px] text-white/50">
          ...
          <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-primary text-[11px] font-black group-hover:gap-3 transition-all uppercase tracking-wider">
    ...
    <section className="py-32 relative">
      <Container>
        ...
        >
          {loading ? (
            [1,2,3].map((i) => (
              <div key={i} className="aspect-[16/10] rounded-3xl bg-white/5 animate-pulse border border-white/10" />
            ))
          ) : ads.length === 0 ? (
            <div className="col-span-full py-20 text-center text-white/30 italic text-xl border border-dashed border-white/10 rounded-3xl">
